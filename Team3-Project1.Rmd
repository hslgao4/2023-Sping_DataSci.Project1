...
```
```{r}
ttest2sample_Clearance_rate = t.test(Robbery_pre$Clearance_rate,Robbery_lock$Clearance_rate)

ttest2sample_Clearance_rate
```
...
```{r}
model = lm(Month~Date, data=Robbery)
summary(model)
```
```{r}
model1 = lm(Month~Time_hour, data=Robbery)
summary(model1)
```

```{r}
model1 = lm(Month~DISTRICT, data=Robbery)
summary(model1)
```

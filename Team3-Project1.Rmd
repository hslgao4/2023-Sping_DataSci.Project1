...
Check the boxplot of daily robbery frequency.

```{r}
ggplot(Robbery_date_sum, aes(x=Period_cat, y=n)) + 
  geom_boxplot( colour=c("#ff0000","#11cc11","#0000ff"), outlier.shape=8, outlier.size=4) + 
  labs(title="Daily robbery between three pendamic phase groups", x="Phase", y = "Frequecy")
```
...
```{r,results='hide'}
str(Robbery_month_sum)
head(Robbery_month_sum,5)
count(Robbery_month_sum,Period_cat)
```
Check the boxplot of daily robbery frequency.

```{r}
ggplot(Robbery_month_sum, aes(x=Period_cat, y=n)) + 
  geom_boxplot( colour=c("#ff0000","#11cc11","#0000ff"), outlier.shape=8, outlier.size=4) + 
  labs(title="Monthly robbery between three pendamic phase groups", x="Phase", y = "Frequecy")
```
...
Check the boxplot of daily average time hour.

```{r}
ggplot(Robbery_avg_time, aes(x=Period_cat, y=avg_Time_hour)) + 
  geom_boxplot( colour=c("#ff0000","#11cc11","#0000ff"), outlier.shape=8, outlier.size=4) + 
  labs(title="Daily avg.robbery avg_Time_hour between three pendamic phase groups", x="Phase", y = "Time(minute)")
```
...
```
Check the boxplot of daily robbery time of clearance.

```{r}
ggplot(Robbery_avg_clr, aes(x=Period_cat, y=avg_Clearance_rate)) + 
  geom_boxplot( colour=c("#ff0000","#11cc11","#0000ff"), outlier.shape=8, outlier.size=4) + 
  labs(title="Daily avg.robbery time to clear between three pendamic phase groups", x="Phase", y = "Time(minute)")
```
...
### 5. Daily robbery district (average `PSA`, Quantitative var)
...
```{r, results='hide'}
#make a summary table
Robbery_avg_psa <-  Robbery_clean %>% group_by(Period_cat, REPORT_DAT_2) %>%
                   summarise(avg_PSA = mean(PSA, na.rm = TRUE),.groups = 'drop')

arrange(Robbery_avg_psa, Period_cat,REPORT_DAT_2) 

Robbery_avg_psa <- data.frame(Robbery_avg_psa)
```

Check the boxplot of daily robbery location.

```{r}
ggplot(Robbery_avg_psa, aes(x=Period_cat, y=avg_PSA)) + 
  geom_boxplot(colour=c("#ff0000","#11cc11","#0000ff"), outlier.shape=8, outlier.size=4) + 
  labs(title="Daily avg.robbery time to clear etween three pendamic phase groups", x="Phase", y = "Time(minute)")
```

```{r}
anova_daily_rob = aov(avg_PSA~Period_cat, data = Robbery_avg_psa)

anova_summary = summary(anova_daily_rob)
anova_summary
#xkabledply(anova_daily_rob)
```
  + F value of the test is **22.8.**
...
## Hypothesis test (Initial version)

**Anova test**

1. anova test between mean of daily robbery frequency and period (pre, lock, and post)
```{r}
anova_dayily_rob = aov(Period_cat_num~n, data = Robbery_date_sum)
xkabledply(anova_dayily_rob)
```
The p-value is 0.857, so we fail to reject the null hypothesis (The Washington DC has same mean of daily robbery frequency in pre-pandemic, lock, and post-pandemic) under the 95% confidence level. 

```{r}
anova_monthly_rob = aov(Period_cat_num~n, data = Robbery_month_sum)
xkabledply(anova_monthly_rob)
```

```{r}
anova_Date_cat_rob = aov(Period_cat_num~n, data = Robbery_Date_cat_sum)
xkabledply(anova_Date_cat_rob)
```

```{r}
anova_district_rob = aov(DISTRICT~avg_Clearance_rate, data = Robbery_district_avg_clr)
xkabledply(anova_district_rob)
```

```{r}
anova_district_clean_rob = aov(DISTRICT~avg_Clearance_rate, data = Robbery_clean_district_avg_clr)
xkabledply(anova_district_clean_rob)
```

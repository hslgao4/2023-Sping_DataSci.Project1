...
Logit_model_2 <- glm(Target_Clearance_time ~ DISTRICT + Period_cat_num_2 + SHIFT + Income_mean  + pct_all_families_below_poverty + worked_from_home_pct , data = data_train_model_1, family = "binomial")
...
ggplot( data_train_model_1, aes( prediction, color = Target_Clearance_time ) ) + geom_density( size = 1 ) +
...
```{r}
# distribution of the prediction score grouped by known outcome
ggplot( data_test_model_1, aes( prediction, color = Target_Clearance_time ) ) + geom_density( size = 1 ) +
...
cm_info <- ConfusionMatrixInfo( data = data_test_model_1, predict = "prediction", 
                                actual = "Target_Clearance_time", cutoff = .55 )
...
```{r}
h <- roc(Target_Clearance_time~prediction, data=data_test_model_1)

plot(h)
```

```{r}
proc_auc2(data_test_model_1, 'Target_Clearance_time', 'prediction')
proc_auc2(data_train_model_1, 'Target_Clearance_time', 'prediction')

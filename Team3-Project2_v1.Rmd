---
title: "Logistic Regression Analysis of Robbery Rates in Washington D.C During COVID-19 Lockdown: Identifying Factors that Influence Police Efficiency and Severity of Robbery"
...
3. What is the impact of incorporating economic and demographic data from the Census in the model, compared to using only crime data?
4. What are some potential use cases for this model in the field of law enforcement, and how could it be integrated into existing crime prevention strategies?
5. What are the most significant predictors of the target variable (Target_robbery_method) in this logistic regression model, and how do they affect the likelihood of the occurrence of the target variable?
6. How accurate is the decision tree model in predicting the Target_robbery_method, and how does it compare to the logistic regression model?


**SMART questions of project 2**
1. What are the most important features that influence the police efficiency (clearance rate) and also the severity of robbery (method)?                     
2. How accurately can we predict the clearance rate and method based on these features?
...
3.  What if police could forecast whether the dangerous districts is?


1. What factors have the strongest impact on the clearance time of robbery incidents, based on the model results?
2.How does the inclusion of economic and demographic data from the Census in the model affect its predictive power compared to using only crime data?
3.What are some potential use cases for this model in the field of law enforcement, and how could it be integrated into existing crime prevention strategies? 

4.What are the most significant predictors of the target variable (Target_robbery_method) in this logistic regression model, and how do they affect the likelihood of the occurrence of the target variable?
...
pairs.panels(model_1_clean[,c(91,3:12)],   #91: Final Target
...
(Pick) Earning_mean, carpooled_pct, drove_alone_pct, Construction_pct

```{r}
pairs.panels(model_1_clean[,c(91,13:20)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
(Pick) hs_200000__pct, Educational_services_and_health_care_and_social_assistance_pct

```{r}
pairs.panels(model_1_clean[,c(91,21:30)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
```{r}
pairs.panels(model_1_clean[,c(91,31:40)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
```{r}
pairs.panels(model_1_clean[,c(91,41:50)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
```{r}
pairs.panels(model_1_clean[,c(91,51:60)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
```{r}
pairs.panels(model_1_clean[,c(91,61:70)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
```{r}
pairs.panels(model_1_clean[,c(91,71:80)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
```{r}
pairs.panels(model_1_clean[,c(91,81:88)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
Following is historam of `Earning_mean`(Census) by the target_Clearance_time groups (1: High, 0: Low)
...
Following is historam of `worked_from_home_pct`(Census) by the target_Clearance_time groups (1: High, 0: Low)
...
Following is historam of `hs_200000__pct`(Census) by the target_Clearance_time groups (1: High, 0: Low)

```{r}

ggplot(model_1_clean, aes(hs_200000__pct, fill = Target_Clearance_time_f)) +  geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity')
ggplot(model_1_clean, aes(hs_200000__pct, fill = Target_Clearance_time_f)) + geom_density(alpha = 0.2)

#Other way
#Make a bin (group) for continous var x and do the same way like category vars 
```

Following is historam of `pct_all_families_below_poverty`(Census) by the target_Clearance_time groups (1: High, 0: Low)


```{r}

ggplot(model_1_clean, aes(pct_all_families_below_poverty, fill = Target_Clearance_time_f)) +  geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity')
ggplot(model_1_clean, aes(pct_all_families_below_poverty, fill = Target_Clearance_time_f)) + geom_density(alpha = 0.2)
...
Following is histogram of `unemployment_rate`(Census) by the target_Clearance_time groups (1: High, 0: Low)

```{r}

ggplot(model_1_clean, aes(unemployment_rate, fill = Target_Clearance_time_f)) +  geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity')
ggplot(model_1_clean, aes(unemployment_rate, fill = Target_Clearance_time_f)) + geom_density(alpha = 0.2)
...
Logit_model_1 <- glm(Target_Clearance_time_f ~ DISTRICT + Period_cat_num_2 + SHIFT + Hour + Season + Date_cat + Day_cat_2 + worked_from_home_pct, data = data_train_model_1, family = "binomial")
...
Regardless of a cutoff level, the AUROC(area-under-curve) is 0.625, which is less than 0.8. The model does not seem to make the best good predict power.
...
target30: Area under the curve: 0.625
...
cm_info <- ConfusionMatrixInfo( data = data_test_model_1, predict = "prediction", 
                                actual = "Target_Clearance_time", cutoff = .5 )
...
library(psych) # pair plots with histogram on diagonal and other options
pairs.panels(model_2_clean[,c(2,3:12)],   #[,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
ggplot(model_2_clean, aes(x = DISTRICT, y = Target_robbery_method)) +
  geom_boxplot(fill = "#00AFBB") +
  labs(x = "District", y = "Robbery Method Target") +
  theme_minimal()
...
**B. Train the model**
```{r}
Model <- glm(Target_robbery_method ~ DISTRICT + + Period_cat_num_2 + Season + Month +SHIFT + drove_alone_pct + Earning_mean + Income_mean + labor_forces_rate + mean_travel_time + unemployment_rate + walked_pct + Sales_and_office_occupations_pct + public_transportation_pct, data = df_train, family = "binomial")

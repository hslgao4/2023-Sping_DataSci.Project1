...
```{r}
pairs.panels(model_1_clean[,c(91,81:89)],   #Target, Census [,c(2:7,9:10)]
             method = "pearson", # correlation method
             hist.col = "#00AFBB",
             density = FALSE,  # show density plots
             ellipses = FALSE # show correlation ellipses
...
plot <- ggplot(temp_gr, aes(DISTRICT, avg)) + geom_line() + labs(x="district", y="Target_robbery_method(% of GUN)") +
  labs(title="Prop. of Gun method by district") + scale_x_discrete(limits=c("1","2","3","4","5","6","7")) + geom_point() #with 3-D: ggplot(temp_gr, aes(x, y, colour = Period_cat))  

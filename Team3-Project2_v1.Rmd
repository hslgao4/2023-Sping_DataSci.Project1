...
**1. `Target_Clearance_time` implys Efficiency of Police or Difficulty of incident, whether need Longer time to clear or not.**
...
A. In terms of using `Period_cat_num` to precit target, we simplifed the period into two categories (Lock-down=1, other=0).
...
B-4. Chart 11 is histogram of `labor_forces_rate`(Census) by the target_Clearance_time groups (1: High, 0: Low)
  ...
  + Interestingly, Higher clearance time robberies (target =1) are more concentrated on the boundary of proportion of working people below 60%, which reveals demographic environment affetct the robbery difficulty.
...
```{r, results='hide'}
plot <- ggplot(data=model_2_clean, aes(Clearance_rate)) + 
  geom_histogram(col="darkgrey", 
                 fill="lightgrey", 
                 alpha = .7,bins = 30) + 
  labs(x="Clearance_rate", y="Frequency") +
...
First, we defined the evaluation functions for convenience.
...
And `r nol(data_train_model_1)-1` features were used as predictors.
...
### 1. Key result: data limitation, relationship with target,
       - Answer of Q1: inference. summary of coefficients, predictors.. 
       - Answer of Q2:  compare performance of three models (Positive:1, Negative: 0)

### 2. Social science things: vilolence, police operation, process, further resarch 

...
Chart C1 shows `Clearance_rate` is highly right skewed, and Chart C2 displaying `avg.Clearance_time` per `DISTRICT` exhibits 
clearance time of robbery visually distinguished by the threshold 40~50 min(similar to average). 
So, We defined Long clearance time as over 40 min and short clearance time as less than 40 min.
...
Our first modeling target, `Target_Clearance_time` has three levels as follows:  
  + `Target_Clearance_time`=1 : Need more than 40 min (near to the average clearance time)
  + `Target_Clearance_time`=0 : Need less than 40 min 
...
To finalize modeling data 1 for `Target_Clearance_time_f`, 
 + The dataset has `r nrow(model_1_clean)` observations, excluded **Outliers of `Target_Clearance_time`** having value Na, 
  (By unusual Observations test using `outlierKD2`in project 1, outliers of `Clearance_rate` that have extremely large value
  were converted into Na).
 
 + And, it has `r ncol(model_1_clean)-2` features and 1 target variable after excluding needless columns for modeling. 
   (`METHOD` also could be used as predictor for model 1).
...
1. Model 1 (`Target_Clearance_time_f`)
 ...
 A.(EDA) Since `Target_Clearance_time_f` came from **Ratio variable, `Clearance_time` that refers duration**, 
  The Box Plots or summarized plots of `Clearance_time` were showed for a few categorical features.
  
 B.(EDA) In terms of interval variables such as Census, Histograms split by the target groups were used for checking relationship.
...
A-1. (EDA-categorical features) Chart 3 indicates `avg.Clearance_time` per `DISTRICTS`. 
  ...
  + It seems to have explicit relationship with target.
...
A-2. Chart 4 indicates `avg.Clearance_time` per `Hour`. 

  + In the evening, after 7pm, clear time seems to slightly increase. 
...
In Chi-squared test of independence between Hour and Target_Clearance_time_f, `Hour` is not statistically significant, but we will use it for prediction.
...
B-2. Chart 9 is histogram of `Income_mean`(Census) by the target_Clearance_time groups (1: High, 0: Low)
  ...
  + Lower clearance time group (target =0) has a tendency of having relatively high income compared with Higher clearance time group. 
...
B-3. Chart 10 is histogram of `worked_from_home_pct`(Census) by the target_Clearance_time groups (1: High, 0: Low)

  + Lower clearance time group has a tendency of having relatively high portion of working at home.
...
ggplot(model_1_clean, aes(worked_from_home_pct, fill = Target_Clearance_time_f)) + geom_density(alpha = 0.2) + labs(title="C10.Distribution of worked_from_home_pct by target groups")

```

B-4. Chart 11 is histogram of `labor_forces_rate`(Census) by the target_Clearance_time groups (1: High, 0: Low)

  + Interestingly, Higher clearance time robberies (target =1) are more concentrated on the boundary of proportion of working people below 60%, which reveals demographic environment affect the robbery difficulty.
...
2. Model 2 (`Target_robbery_method`)
 ...
 A.(EDA) Since `Target_robbery_method` is **Binary Quantitative variable**, plots that showing percentage of Gun method(target =1) ware showed for a few categorical features.
  
 B.(EDA) Similarly with model 1, histograms split by the target groups were used for checking relationship between interval variables and target.
...
## Ⅴ.Model building and Evaluation
...
And `r ncol(data_train_model_1)-1` features were used as predictors.



**B. Train the model**

Next, we move on to the training stage of the logistic regression model. We begin by training the initial model, which includes all the predictor variables. We then perform a significance test on each variable and exclude any variable that does not demonstrate a statistically significant impact on the target variable, as indicated by its P-value. Once we have removed these insignificant variables, we re-fit the model with the remaining predictor variables. After this process, we are left with a final set of five predictor variables that demonstrate a significant impact on the target variable. 
...
In this logistic regression model, we aim to predict Target_Clearance_time_f using a combination of categorical and continuous predictor variables, such as DISTRICT, Season, Income_mean, and mean_travel_time. The binary value of 0 or 1 represents whether the clearance time was longer or shorter than the median clearance time for a specific crime category.
...
The significant predictors of the target variable are DISTRICT, Day_cat_2, Period_cat_num_2, SHIFT, and METHOD. In contrast, Date_cat, Season, and Income_mean are not significant predictors. The model has some predictive power, as indicated by a lower residual deviance compared to the null deviance. However, the relatively high AIC value suggests that the model may not fit the data well. Moreover, the model tends to overestimate the probability of longer clearance times, as indicated by negative deviance residuals for most data points. Therefore, further model evaluation and improvement may be necessary before using this model for prediction or inference.

*Overall, based on the GLM model, the most important features that influence the clearance time are:*
1. `DISTRICT`: As the crime rate in the district increases, the clearance time increases (positive coefficient).

2. `Period_cat_num_21`: The clearance time is higher during the 21st period compared to other periods (positive coefficient).

3. `SHIFTEVENING` and `SHIFTMIDNIGHT`: The clearance time is higher during evening and midnight shifts compared to the morning shift (positive coefficients).

4. `Hour`: With each additional hour, the clearance time increases (positive coefficient).

5. `Day_cat_2Weekend`: The clearance time is higher during weekends compared to weekdays (positive coefficient).

6. `worked_from_home_pct`: As the percentage of officers who worked from home increases, the clearance time decreases (negative coefficient).
...
We quickly evaluated AUROC of the fitted model.
...
The results of Model 1 indicate that the area under the receiver operating characteristic (AUROC) curve is 0.642, regardless of the chosen cutoff level. This value is less than 0.8, which is generally considered to be an acceptable level of predictive power. Therefore, it can be inferred that the model does not seem to be very effective in accurately predicting outcomes. Further analysis and model improvements may be necessary to increase its predictive power. 
```{r}
...
The intercept coefficient is positive, indicating that when all other predictors are held constant, the log odds of high_diff_rob is positive.

Several predictor variables have coefficients that are statistically significant, with p-values less than 0.05. For instance, the coefficients of Day_cat_2, Period_cat_num_2, SHIFT, DISTRICT, drove_alone_pct, hs_10000_to_14999_pct, management_business_science_arts_occupations_pct, mean_travel_time, and Transportation_and_warehousing_and_utilities_pct are all positive and statistically significant, suggesting that these variables are significant predictors of the high_diff_rob.

On the other hand, some variables such as hs_100000_to_149999_pct, hs_15000_to_24999_pct, hs_150000_to_194999_pct, hs_200000__pct, hs_Less_than_10000_pct, Income_mean, service_occupations_pct, Social_Security_income_mean, unemployment_rate, and worked_from_home_pct have coefficients that are not statistically significant, suggesting that these variables are not significant predictors of the high_diff_rob.


**C. Evaluate the model**

C-1. Determining the cut-off
After analyzing the probability distribution charts for each target group, a cut-off value of 0.2 was selected as the criterion for predicting the target variable as 1.
...
C3- Calculate True Positives, True Negatives, False Positives, and False Negatives.
...
* The value of 1.590698 for accuracy is unusual as accuracy is typically between 0 and 1. It is possible that there is an error in the calculation or the code used to produce this output.

* The value of 0.5830178 for precision indicates that out of all the predicted positive cases (true positive and false positive), only about 58% were actually positive.

* The value of 0.6435821 for recall rate indicates that out of all the actual positive cases (true positive and false negative), about 64% were correctly identified by the model as positive.

* The value of 0.6118048 for F1 score is the harmonic mean of precision and recall, and represents a balance between the two metrics.

* The value of 0.4418338 for FPR (false positive rate) indicates the proportion of negative cases (true negative and false positive) that were incorrectly classified as positive by the model.

* The value of 0.3564179 for FNR (false negative rate) indicates the proportion of positive cases (true positive and false negative) that were incorrectly classified as negative by the model.
...
C-4 Evaluate per target levels

Now, we creates two data frames df_neg and df_pos by subsetting temp based on the actual target values. df_neg contains all the observations where the actual target value is 0 (negative class), and df_pos contains all the observations where the actual target value is 1 (positive class). This allows us to evaluate the model's performance separately for each target class.
...
Based on the evaluation results, it appears that the model performs better in predicting the negative class (target=0, Low time) compared to the positive class (target=1, High time). For the negative class, the model has an accuracy of 1.41, which is higher than the accuracy of 0.474 for the positive class. The precision for the negative class is 0, indicating that there were no false positives, while the precision for the positive class is 1, indicating that all positive predictions were correct.

However, the recall rate and F1 score for the negative class are NaN (not a number), which means that these measures could not be calculated due to the absence of true positives. In contrast, the recall rate and F1 score for the positive class are 0.474 and 0.643, respectively.

The false positive rate (FPR) for the negative class is 0.44, indicating that there were some false positive predictions. The false negative rate (FNR) for the positive class is 0.526, indicating that over half of the actual positive cases were not correctly identified by the model. Overall, these results suggest that the model is better at predicting the negative class than the postive. 
...
The residual deviance is 8087.9 on 6193 degrees of freedom, indicating that the model fits the data well. The AIC value is 8117.9, which is a measure of the model's quality compared to other models, with lower values indicating better fit. The null deviance is the deviance of a model with only an intercept, and the difference between the null and residual deviances represents the amount of deviance explained by the model's independent variables.
...
`public_transportation_pct`: This variable also has a negative coefficient and odds ratio less than 1, indicating that areas with a higher proportion of people using public transportation are associated with lower likelihood and severity of robbery methods.

`Sales_and_office_occupations_pct`: This variable has a positive coefficient and odds ratio greater than 1, indicating that areas with a higher proportion of people working in sales and office occupations are associated with higher likelihood and severity of robbery methods.

`DISTRICT`: This variable has a small but positive coefficient and odds ratio greater than 1, indicating that some districts have a slightly higher likelihood and severity of robbery methods compared to others.

`mean_travel_time`: This variable has a positive coefficient and odds ratio greater than 1, indicating that longer travel times are associated with higher likelihood and severity of robbery methods.
...
The AUC 0.664, which suggests that the model has a moderate level of discrimination power in distinguishing between positive and negative cases. However, it is difficult to make a judgment about the quality of the model solely based on the AUC value without considering other factors.
...
H1- Accurcy, Precision, Recall when cut-off 0.5
...
Based on the given metrics, it appears that the model for Target_robbery_method has an accuracy of 0.618, which means that 61.8% of the predictions made by the model are correct. The precision of the model is 0.616, which means that 61.6% of the instances that the model predicted as positive are actually positive. The recall rate is 0.593, which means that the model correctly identifies 59.3% of the positive instances. The F1 score is 0.604, which is the harmonic mean of precision and recall and provides a balance between the two metrics.

The false positive rate (FPR) is 0.352, which means that 35.2% of the instances that are actually negative are incorrectly predicted as positive by the model. The false negative rate (FNR) is 0.407, which means that 40.7% of the instances that are actually positive are incorrectly predicted as negative by the model. These metrics provide a more detailed understanding of the model's performance.


H2- Evaluate per target levels

Now, we will redo the same process which recreates two data frames df_neg and df_pos by subsetting temp based on the actual target values. df_neg contains all the observations where the actual target value is 0 (negative class), and df_pos contains all the observations where the actual target value is 1 (positive class). 
...
We Will use the same variables with logistic regression.
...
The results show that the AUC for the test set is 0.602, while the AUC for the training set is slightly higher at 0.614. AUC is a measure of how well a model can distinguish between positive and negative instances, with a higher AUC indicating a better model performance. However, the fact that the AUC is higher for the training set compared to the test set suggests that the model may be overfitting to the training data and may not generalize well to new, unseen data.


## Ⅵ.Conclusion

In conclusion, our project aimed to accurately predict clearance time and robbery severity by identifying the key features that influence these outcomes. Our exploratory data analysis uncovered several important insights, such as the impact of district, shift, day of the week, and time of day on clearance time, and the increase in gun robberies during lockdown. Using logistic regression models, we found that DISTRICT and Lockdown contribute to the longer clearance time, while SHIFT(MIDNIGHT) and drove_alone_pct have the most positive effect to the gun method. We achieved reasonable performance, with AUC values of 0.6424 and 0.664 for predicting clearance time and robbery method severity, respectively. While predicting criminal behavior is a complex task, our models provide a starting point for understanding the factors that influence it. Our SMART questions involved both inference and prediction, allowing us to interpret significant predictors and evaluate the models' performance.

Furthermore, our project highlights the broader implications for social science research related to violence and policing. Understanding the factors that influence criminal behavior and the effectiveness of police operations is crucial for addressing issues of public safety and crime prevention. Continued research in this area can identify best practices for policing and provide insights into the social, economic, and cultural factors that contribute to criminal activity.

It is essential to recognize the ethical implications of using data and models to make decisions related to law enforcement and ensure that these tools are used in a fair and unbiased manner. As new data becomes available, it is crucial to continue research and analysis in this area to develop more effective strategies for crime prevention and public safety.

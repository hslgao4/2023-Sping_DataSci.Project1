...
Next, we move on to the training stage of the logistic regression model. We begin by training the initial model, which includes all the predictor variables. We deleted some variables which are not significant, and re-fit the model with the remaining predictor variables. 
...
In this logistic regression model, we aim to predict Target_Clearance_time_f using a combination of categorical and continuous predictor variables, such as DISTRICT, Season, Income_mean, and mean_travel_time. 
...
The results of Model 1 indicate that the area under the receiver operating characteristic (AUROC) curve is 0.642. This value is less than 0.8, which is generally considered to be an acceptable level of predictive power. Therefore, it can be inferred that the model does not seem to be very effective in accurately predicting outcomes. Further analysis and model improvements may be necessary to increase its predictive power. 
```{r}
...
Several predictor variables have coefficients that are statistically significant, with p-values less than 0.05. For instance, the coefficients of Day_cat_2, Period_cat_num_2, SHIFT, DISTRICT, and drove_alone_pct are all positive and statistically significant.

On the other hand, some variables such as hs_100000_to_149999_pct, hs_15000_to_24999_pct, and hs_150000_to_194999_pct are not significant predictors of the high_diff_rob.
...
Now, we creates two data frames df_neg and df_pos by subsetting temp based on the actual target values. 
...
Based on the evaluation results, it appears that the model performs better in predicting the negative class (target=0, Low time) compared to the positive class (target=1, High time). 
